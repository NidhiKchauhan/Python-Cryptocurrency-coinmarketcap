...
table = PrettyTable(['Asset', 'Amount Owned', convert + ' Value', 'Price', '1h (change)', '24h (change)', '7d (change)'])

with open('portfolio.txt') as inp:
    for line in inp:
        ticker, amount = line.split() #first word as ticker and after that (word) as amount
        ticker = ticker.upper()
